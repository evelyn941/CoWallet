        ...
        <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
          <div className="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setSelectedGroupId(null)}>
              ...
              <span className="text-xl font-semibold tracking-tight">SplitWise</span>
            </div>
...
const GroupDetail = ({ groupId, user, onBack }: { groupId: string, user: UserProfile, onBack: () => void }) => {
  ...
  const handleDismissGroup = async () => {
    ...
    if (!window.confirm('Dismissing the group will delete all expenses. Continue?')) return;
    try {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {group.memberIds.map(id => (
                <div key={id} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between group/member">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-sm font-bold text-gray-400 uppercase">
                      {(group.memberNames?.[id] || 'U').charAt(0)}
                    </div>
                    <div>
                  ...
                  {debts.map((debt, idx) => (
                    <div key={idx} className="flex flex-col gap-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium text-gray-500">{group.memberNames?.[debt.from] || 'Unknown'}</span>
                        <span className="text-gray-300">owes</span>
                        <span className="font-medium text-gray-500">{group.memberNames?.[debt.to] || 'Unknown'}</span>
                      </div>
                      <div className="text-2xl font-bold text-black">
                        ${debt.amount.toFixed(2)}
                      </div>
                      {idx < debts.length - 1 && <div className="h-px bg-gray-50 mt-2" />}
                    </div>
                  ))}
                </div>
              )}
